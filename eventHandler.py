...
class EventHandler(AbstractEventListener):
    ...
    def before_find(self, by, value, driver):
        ...
        if by == By.XPATH:
            if "//" in value or "/html" in value or "//html" in value:
                ...
                if not element_name:
                    element_name = element.get_attribute("name") or element.get_attribute(
                        "id") or element.get_attribute("class") or "unknown_element"
                value = f"{value}[contains(text(), '{element_name}')]"
                event_text = f"Element found {element_name}"
            else:
                element = driver.find_element(By.XPATH, value)
                element_name = element.text.strip()
    ...
    def after_change_value_of(self,element:WebElement,driver:WebDriver):
        self._log_event(f"Value entered {element.get_attribute('value')}")
    def before_execute_script(self, script, driver):
        # self._log_event(f"Before executing script: {script}")
        self._log_event(f"Scrolling done before")

    def after_execute_script(self, script, driver):
        self._log_event(f"Scrolling done after")
