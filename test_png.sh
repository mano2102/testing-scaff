...
cp "/home/coder/project/workspace/selenium/eventhandlerdummy.py" "$base_dir/utilities/eventhandler.py"
...
if [ "$line_count" -gt 150 ]; then
    echo "The log file has more than 150 lines"
else
    echo "The log has 150 or fewer lines."
fi

# Search for specific log content
if [ -f "$logfile" ] && grep -q "Pincode Entered : 400001" "$logfile"; then
    echo "Found 'Pincode Entered : 400001' in log file"
else
...
if [ -f "$reportfile" ]; then
    ...
    if [ "$line_count" -gt 150 ]; then
        echo "The report file has more than 150 lines."
    else
        echo "The report file has 150 or fewer lines."
    fi
...
# check for screenhot my name
check_screenshot_by_name() {
    local scr_dir=$1
    local prefix=$2

    local match=$(find "$scr_dir" -type f -name "${prefix}*.png" | head -n 1)

    if [ -n "$match" ]; then
        echo "Screenshot '$prefix' exists"
    else
        echo "No screenshot starting with '$prefix' found."
    fi
}


check_screenshot_by_name "$scrdir" "gold_earring_kids"
